...
describe('scoring', () => {
	describe('convertCardToNumber()', () => {
		it('should return the correct numeric value for a card', () => {
			...
			cards.forEach((card, index) =>
				expect(convertCardToNumber(card)).toBe(index)
			);
		});
	});
	...
	describe('isFiveOfAKind()', () => {
		test('it should return true when the hand has 5 cards of equal value', () => {
			expect(isFiveOfAKind(groupCards([1, 1, 1, 1, 1]), 0)).toBe(true);
			expect(isFiveOfAKind(groupCards([1, 1, 1, 1]), 1)).toBe(true);
			expect(isFiveOfAKind(groupCards([1, 1, 1]), 2)).toBe(true);
			expect(isFiveOfAKind(groupCards([1, 1]), 3)).toBe(true);
			expect(isFiveOfAKind(groupCards([1]), 4)).toBe(true);
			expect(isFiveOfAKind(groupCards([]), 5)).toBe(true);
		});
		test('it should return false when the hand does not have 4 cards of equal value', () => {
			expect(isFiveOfAKind(groupCards([1, 1, 2, 1, 1]), 0)).toBe(false);
			expect(isFiveOfAKind(groupCards([1, 1, 2, 1]), 1)).toBe(false);
			expect(isFiveOfAKind(groupCards([1, 2, 1]), 2)).toBe(false);
			expect(isFiveOfAKind(groupCards([1, 2]), 3)).toBe(false);
		});
	});

	describe('isFourOfAKind()', () => {
		test('it should return true when the hand has 5 cards of equal value', () => {
			expect(isFourOfAKind(groupCards([2, 1, 1, 1, 1]), 0)).toBe(true);
			expect(isFourOfAKind(groupCards([1, 2, 1, 1]), 1)).toBe(true);
			expect(isFourOfAKind(groupCards([1, 1, 2]), 2)).toBe(true);
			expect(isFourOfAKind(groupCards([2, 1]), 3)).toBe(true);
			expect(isFourOfAKind(groupCards([1]), 4)).toBe(true);
		});
		// test('it should return false when the hand does not have 4 cards of equal value', () => {
	...
	describe('isFullHouse()', () => {
		test('it should return true when the hand has 3 and 2 cards of equal value', () => {
			expect(isFullHouse(groupCards([1, 1, 1, 2, 2]), 0)).toBe(true);
			expect(isFullHouse(groupCards([1, 2, 1, 2]), 1)).toBe(true);
			expect(isFullHouse(groupCards([2, 1, 2]), 2)).toBe(true);
			expect(isFullHouse(groupCards([2, 1]), 3)).toBe(true);
			expect(isFullHouse(groupCards([2]), 4)).toBe(true);
		});
