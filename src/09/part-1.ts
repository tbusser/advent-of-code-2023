...
function findNextValue(history: string): number {
	...
	do {
		...
		lastValues.push(current.at(-1));
	} while (current.some(value => value !== 0) && current.length > 1);

	// Add up all the last values of the generated sequences, the sum is the
	// predicted next value based on the history.
	return calculateTotal(lastValues);
}
